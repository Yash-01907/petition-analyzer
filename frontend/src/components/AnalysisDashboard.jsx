...
export default function AnalysisDashboard({ result, onAnalysisComplete }) {
  ...
  const handleRetrainUpload = async (event) => {
    ...
    try {
      ...
      setRetrainMessage(
        retrainResult?.message || "Retraining complete. Dashboard refreshed.",
      );
    } catch (err) {
      setRetrainError(
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-sm font-medium text-gray-500 mb-1">
            Top Performing Traffic
          </h3>
          <p className="text-3xl font-bold text-gray-900 capitalize">
            {source_breakdown?.length > 0
              ? source_breakdown.reduce(
                  (max, s) => (s.avg_conversion > max.avg_conversion ? s : max),
                  source_breakdown[0],
                ).traffic_source
              : "—"}
          </p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
        ...
        <p className="text-sm text-gray-500 mb-6">
          The machine learning model identified these features as having the
          highest impact on conversion rates in your historical data.
        </p>
        <div className="h-[22rem] sm:h-80 w-full text-sm">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={
                isMobile ? feature_importance.slice(0, 8) : feature_importance
              }
              layout="vertical"
              margin={
                isMobile
                  ? { top: 5, right: 10, left: 110, bottom: 5 }
            ...
            {archetypes.map((arch, index) => (
              ...
              >
                ...
                <div className="text-xs text-gray-500 mb-2">
                  ({arch.campaign_count} campaigns)
                </div>
                <div className="flex flex-wrap gap-1 mb-3">
                  {arch.dominant_traits.map((trait, i) => (
                    <span
                      key={i}
                      className="text-[10px] bg-white border border-gray-200 px-2 py-0.5 rounded-full text-gray-600"
                    >
                      {trait}
                    </span>
                  ))}
                  ...
                  <tr key={index}>
                    <td className="px-3 py-3 whitespace-normal">
                      ...
                      <span className="text-xs text-gray-400 block">
                        {score.campaign_id} • {score.traffic_source}
                      </span>
                    </td>
                    <td className="px-3 py-3 whitespace-nowrap">
                      <span className="text-sm text-gray-900 font-bold">
                        {score.actual_conversion}%
                      </span>
                    </td>
                    <td className="px-3 py-3 whitespace-nowrap">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${GRADE_COLORS[score.grade] || "bg-gray-100 text-gray-800"}`}
                      >
                        {score.grade}
                      </span>
                    </td>
